...
{
    public sealed class GUIXMLLoader
    {
        ...
        public static void LoadGUIXML(string xmlPath, string xmlTag, ref List<XMLEle> xmlEle)
        {
            ...
            if (File.Exists(xmlPath))
            {
                ...
                XmlNode root = m_xmlDoc.SelectSingleNode("ContentLayout");
                XmlNode loadRoot = root.SelectSingleNode(xmlTag);
                LoadXMLNode(loadRoot, XMLSTYLE.UNITY_EditorGUI, ref xmlEle);
            }
        }

        public static void LoadXMLNode(XmlNode xn, XMLSTYLE editorGUIlayout, ref List<XMLEle> xmlEle)
        {
            ...
            foreach (XmlNode xnItem in rootList)
            {
                ...
                {
                    LoadXMLNode(xnItem, XMLSTYLE.UNITY_EditorGUILayout, ref xmlEle);
                }
                else if (type == "EditorGUI")
                {
                    LoadXMLNode(xnItem, XMLSTYLE.UNITY_EditorGUI, ref xmlEle);
                }
                else if (type == "GUI")
                {
                    LoadXMLNode(xnItem, XMLSTYLE.UNITY_GUI, ref xmlEle);
                }
                else if (type == "GUILayout")
                {
                    LoadXMLNode(xnItem, XMLSTYLE.UNITY_GUILayout, ref xmlEle);
                }
                else
                {
                    XMLEle xe = new XMLEle();
                    xe.m_editorGUILayout = editorGUIlayout;
                    xe.m_type = type;
                    ...
                    {
                        xe.m_position = new Rect(float.Parse(posX), float.Parse(posY), float.Parse(width), float.Parse(height));
                    }

                    //Add GUI Layout Option
                    #region GUI Layout Option Init
                    List<GUILayoutOption> option = new List<GUILayoutOption>();
                    if (width != "")
                    {
                        option.Add(GUILayout.Width(float.Parse(width)));
                    }

                    if (height != "")
                    {
                        option.Add(GUILayout.Height(float.Parse(height)));
                    }

                    if (option.Count > 0)
                    {
                        xe.m_layoutOptions = option.ToArray();
                    }
                    #endregion
...
//                     if (xe.m_type != "IntPopup")
//                     {
//                         xmlEle.Add(xe);
//                     }
//                     else
//                     {
//                         Debug.Log("XML Loader xe:");
//                     }
                    xmlEle.Add(xe);
        ...
        public static void XMLDraw(XMLEle element)
        {
            ...
            if (element.m_editorGUILayout == XMLSTYLE.UNITY_EditorGUI)
            {
                    ...
                    case "LabelField":
                        {
                            ...
                            {
                                EditorGUI.LabelField(element.m_position, element.m_content);
                            }
                            break;
                        }
                    case "LayerField":
            ...
            else if (element.m_editorGUILayout == XMLSTYLE.UNITY_EditorGUILayout)
            {
                    ...
                    case "IntPopup":
                        {
                            ...
                            {
                                element.m_value = EditorGUILayout.IntPopup(element.m_content, (int)element.m_value, element.m_displayedOptions, element.m_optionValues, element.m_layoutOptions);
                            }
                            else
                            {
            ...
            else if (element.m_editorGUILayout == XMLSTYLE.UNITY_GUI)
            {
                #region GUI Options

                #endregion
            }
            else if (element.m_editorGUILayout == XMLSTYLE.UNITY_GUILayout)
            {
                #region GUILayout Options
                switch (element.m_type)
                {
                    case "Button":
                        element.m_value = GUILayout.Button(element.m_content, element.m_layoutOptions);
                        break;
                    case "FlexibleSpace":
