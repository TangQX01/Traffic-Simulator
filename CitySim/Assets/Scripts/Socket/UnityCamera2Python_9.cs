...
namespace CitySim{
    ...
    public class UnityCamera2Python_9 : MonoBehaviour
    {
        ...
        void Update()
        {
            if (isStartCoroutine == false && iscon == true)
            {
                ...
                StartCoroutine(CameraTexture2Python());
        ...
        public void CameraCapture( Camera camera)
        {
            ...
            //SendMessage_9 CM = new SendMessage_9();
            ////CM.image_bytes = bytes;
            //CM.camera_name = camera.name;
            //CM.image_bytes = Encoding.UTF8.GetString(bytes);
        ...
        public IEnumerator CameraTexture2Python()
        {
            //CameraCapture(camera);
            //print(camera.name + "截图已上传给python");
            //yield return null;
            while (true)
            {
                yield return new WaitUntil(() => isRecv == true);
                //yield return new WaitForSecondsRealtime(1);
                CameraCapture(m_CamList[m_CameraId]);
                print(m_CamList[m_CameraId].name + "截图已上传给python");
                m_CameraId += 1;
                m_CameraId %= 6;
                isRecv = false;
            }
