...
public sealed class ObjectDetector : System.IDisposable
{
    ...
    void RunModel(Texture source, float threshold)
    {
        ...
        //Debug.Log("NN worker invocation");
        using (var t = new Tensor(_config.InputShape, _buffers.preprocess))
            _worker.Execute(t);

        /// <summary>
        /// Tqx: output和197是ONNX模型的OUTPUT_NAME
        /// </summary>
        // NN output retrieval
        //Debug.Log("NN output retrieval");
        //Debug.Log(_buffers.feature1.GetType());
        _worker.CopyOutput("output", _buffers.feature1);
        _worker.CopyOutput("197", _buffers.feature2);
        //Debug.Log("End NN output retrieval");

        // Counter buffer reset
        //Debug.Log("Counter buffer reset");
        _buffers.post2.SetCounterValue(0);
        _buffers.counter.SetCounterValue(0);

        // First stage postprocessing: detection data aggregation
        //Debug.Log("First stage postprocessing: detection data aggregation");
