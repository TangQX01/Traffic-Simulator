...
def get_args_parser():
    ...
    # Dataset parameters.
    parser.add_argument('--dataset_file', default='hoia')

    parser.add_argument('--model_path', default=r"E:\1-Project\riding-violation\HOI\HoiTransformer\checkpoint0299.pth",
    ...
    # Visualization.
    parser.add_argument('--img_sheet', default=None, help=r"E:\1-Project\riding-violation\HOI\HoiTransformer\data\hoia\images\test")
    return parser
...
def predict_on_one_image(args, model, device, img_tensor, img_size, hoi_th, human_th, object_th, top_k=100):
    ...
    for idx_box, idx_action in box_action_pairs:
        ...
        for idx_box, hoi in enumerate(hoi_list):
            #human
            x11, y11, x22, y22 = hoi['h_box']
            color_h = hoi['color']
            if abs(x1-x11)<=30 and abs(x2-x22)<=30 and abs(y1-y11)<=30 and abs(y2-y22)<=30:
                h_box[0] = x11
                h_box[1] = y11
                h_box[2] = x22
                h_box[3] = y22
        ...
        pp = dict(
            ...
            h_box=h_box, o_box=o_box, i_box=i_box, h_name=h_name, o_name=o_name, i_name=i_name, h_no=0, color = color
        )
        hoi_list.append(pp)
...
def main():
    ...
    if args.img_sheet is None:
        img_path_list = [
            # './data/hico/images/train2015/HICO_train2015_00000001.jpg'
            r'E:\1-Project\riding-violation\HOI\HoiTransformer\data\hoia\images\test\test45.jpg',
            r"E:\1-Project\riding-violation\HOI\HoiTransformer\data\hoia\images\mytest\test01.jpg"
        ]
    else:
        # img_path_list = ['./test61.jpg']
        img_path_list = [l.strip() for l in open(args.img_sheet, 'r', encoding='utf-8').readlines()]
